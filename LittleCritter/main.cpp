...
void rightWallFollow(vector<point> sensorData){
	...
	ftime(& currentTime);
	if(!frontWall(sensorData)){
		double diff=250-min.r;
		double timeDiff= (currentTime.time*1000+currentTime.millitm)-(oldTime.time*1000+oldTime.millitm);
		double velocity=(min.r-oldMin.r)/timeDiff;

		if(fabs(diff)>250){
			diff=(diff<0?-1:1)*250;
		}
		setSpeed(500-diff/2.5+velocity*35 , 500+diff/2.5-velocity*35 );
	}
	else{
		spinLeft();
	}
	...
	oldMin=min;
        /*if((min.r<250 || min.r>9000) && !frontWall(sensorData)){
		    setSpeed(500-(250-min.r)/2, 500+(250-min.r)/2); // 450/520
		    //cout<<"left"<<endl;
	    }
	      ...
	      // cout<<"spin"<<endl;
		    ...
		    setSpeed(500, 500);//650/420
	    }
	    else{
		    setSpeed(500+(min.r-250)/2, 500-(min.r-250)/2); // 550/440
		    //cout<<"right"<<endl;
	    }  */  
